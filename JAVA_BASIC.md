...
- 자바 소스코드 구조
    ...
    /* 여러줄 
       주석. */
    package com.leeyb98.spring01; // 자신 프로젝트의 패키지명(폴더)
    ...
    public class Spring01Application { // 자신 클래스명
        ...
        public static void main(String[] args) { // 함수, 클래스, 조건문, 반복문시작이 중괄호({)
            ...
            System.out.print("Hello, Spring Boot!"); // 콘솔출력, 문자열을 항상 "
        }
    }
    ```
...
- JVM위에서 동작
    - Java Virtual Machine : 도커 컨테이너와 비슷한 개념념
    - sourcecode.java -> javac(java compiler) -> bytecode.class -> java로 실행

- 활용분야
    - 웹이 특화. 인터넷 포털, 인터넷 뱅킹, 쇼핑몰, 기업시스템
...
- 원시자료형(Primitive type)
    ...
    |논리형|boolean|1byte, true또는flase |논리형, 참/거짓표현용 |
...
- 조건(분기)문 : if문, switch문
    ...
    switch (입력변수) {
        ...
        default:
            기본실행코드 블럭;
            break; // case나 default에 되도록이면 빼지말것것
    }
    ```
